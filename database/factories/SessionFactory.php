...
class SessionFactory extends Factory
{
    ...
    public function definition(): array
    {
        ...
        $halls = DB::table('halls')->whereIn('name', $hallData['name'])->whereIn('rows', $hallData['rows'])
            ...
            ->whereIn('price_vip_chair', $hallData['price_vip_chair'])->pluck('id')->toArray();

        $keyHall = array_rand($halls);

        $filmData = AppHelper::instance()->getFilmData();

        $films = DB::table('films')->whereIn('title', $filmData['titles'])->whereIn('description', $filmData['description'])
            ->whereIn('movie_duration', $filmData['movie_duration'])->whereIn('image', $filmData['image'])
            ->whereIn('country_of_origin', $filmData['country_of_origin'])->pluck('id')->toArray();
